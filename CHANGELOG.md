...
## 2026-06-10
- 聊天页面加入模式 编码 和计划模式 ，计划模式下将不会修改代码
- 软件研发页面加入AI方案交流功能，有时对于方案中的一些技术不明白，用生成方案的方式聊天时上下文比较大 ，并且AI擅自改方案，加入AI方案交流目的时 方案中的一些疑问分析 最终不会修改方案和代码
交流清楚后，再回到AI生成方案中指导AI修改
- 加入新建子方案功能，当一个方案实施完后，可能遗忘了某种情况这是需要快速开始新方案，继续出方案，这是一种快速方式

## 2026-06-24
- 升级到1.0.49版本，修复数据库退出时可能丢失数据的问题
- 软件研发页面输入框样式统一优化，命令、选择文件、选择Skill操作更便捷
- 软件研发页面聊天区滚动条美化
- 方案设计页面编辑器顶部新增保存、刷新、新建子方案按钮
- 开发记录面板和聊天面板支持拖拽调整宽度
- 方案交流页面输入框样式统一
- 修复Design页面布局溢出导致底部状态栏被遮挡的问题
- Design页面标题栏实时显示AI状态（处理中⏳ / 已完成✅）
- Design侧边栏拖拽范围扩大，最大可拉到800px
- Design页面树底部留白增加，避免被输入框遮挡
- 软件研发页面支持URL参数记忆，刷新或分享链接保持当前状态