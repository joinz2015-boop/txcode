...
import { getFileContent, writeFile, createSession, deleteSession } from '@/api/index'
...
export function useSession() {
  ...
  async function readSessionJson(basePath) {
    try {
      const res = await getFileContent(getSessionJsonPath(basePath))
  ...
  async function writeSessionJson(basePath, data) {
    await writeFile(getSessionJsonPath(basePath), JSON.stringify(data, null, 2))
  }
  ...
  async function deleteSessionLocal(session, basePath) {
    try {
