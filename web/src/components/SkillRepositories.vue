  ...
  <div class="skill-repositories">
    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <div class="skills-panel border-t border-border" :class="{ 'hidden': !expandedRepos.has(repo.id) }">
          <div class="p-3 bg-black/20">
            ...
            <div v-else>
              <div class="flex justify-between items-center mb-2">
                ...
                <el-button
                  v-if="repoSkills[repo.id] && repoSkills[repo.id].length > 0"
                  type="text"
                  size="small"
              ...
              <div class="space-y-1">
                <div v-if="loadingSkills[repo.id]" class="text-textMuted text-xs">
                  <i class="fa-solid fa-spinner fa-spin mr-1"></i> Loading skills...
                </div>
                <div v-else-if="!repoSkills[repo.id] || repoSkills[repo.id].length === 0" class="text-textMuted text-xs">
                  No skills found in this repository.
                </div>
                <div
                  v-else
                  v-for="skill in repoSkills[repo.id]"
                  :key="skill.name"
                  class="flex items-center justify-between py-1"
                >
                  <div class="flex items-center gap-2">
                    <span class="text-sm text-textMain">{{ skill.name }}</span>
                    <el-tag v-if="skill.license" type="warning" size="mini">{{ skill.license }}</el-tag>
                  </div>
                  <el-button type="text" size="small" @click="$emit('download-skill', repo.id, skill.name)">Install</el-button>
                </div>
              </div>
            </div>
          </div>
  ...
  methods: {
    toggleExpand(repoId) {
      if (this.expandedRepos.has(repoId)) {
        ...
        if (!this.repoSkills[repoId]) {
          this.$emit('load-skills', repoId)
        }
      }
