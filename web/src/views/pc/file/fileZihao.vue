...
export default {
  ...
  methods: {
    ...
    async uploadToZihao() {
      ...
      try {
        const response = await fetch(`/api/file/download_file?path=${encodeURIComponent(target.path)}`)
    ...
    async handleZihaoDrop(e) {
      ...
      this.uploadProgress = { visible: true, percent: 0, filename, isDownload: false }
      try {
        const response = await fetch(`/api/file/download_file?path=${encodeURIComponent(this.dragFile.path)}`)
        if (!response.ok) throw new Error('读取文件失败')
        const blob = await response.blob()
        const file = new File([blob], filename)
