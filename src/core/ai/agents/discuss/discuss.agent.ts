...
async function loadRoleTemplate(): Promise<string> {
  try {
    ...
    return '你是 txcode 的 AI 方案探讨助手，专门帮助用户进行方案分析、技术讨论和问题解答。你不能修改任何文件。';
  }
}

async function buildDiscussPrompt(
...
export class DiscussionAgent implements AIProvider {
  name = 'discuss';
  tools = DISCUSS_TOOLS;
  keepContext = true;
  ...
  constructor(config: DiscussionAgentConfig) {
    ...
    this.toolRegistry = new AgentToolRegistry(DISCUSS_TOOLS, { verboseError: true });
  }
  ...
  async run(
    ...
    const systemPrompt = await buildDiscussPrompt(this.maxIterations, {
    ...
    this.injectedUserMessage = userMessage;
