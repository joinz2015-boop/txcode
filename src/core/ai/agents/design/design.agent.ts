...
import { DESIGN_TOOLS } from './agent_tool.js';
...
async function loadRoleTemplate(): Promise<string> {
  try {
    ...
    return '你是 txcode 的 AI 设计助手，专门帮助用户完成前端页面设计任务。';
  }
}

async function buildDesignPrompt(
...
export class DesignAgent implements AIProvider {
  name = 'design';
  tools = DESIGN_TOOLS;
  keepContext = true;
  ...
  constructor(config: DesignAgentConfig) {
    ...
    this.toolRegistry = new AgentToolRegistry(DESIGN_TOOLS, { verboseError: true });
  }
  ...
  async run(
    ...
    const systemPrompt = await buildDesignPrompt(this.maxIterations, {
