...
export const globTool: Tool = {
  ...
  execute: async (params: { pattern: string; directory?: string }, context: ToolContext): Promise<ToolResult> => {
    ...
    const output = `Found ${results.length} files${results.length > maxShow ? ` (showing first ${maxShow})` : ''}:\n${shown.join('\n')}\n\nDirectory: ${dir}${results.length > maxShow ? `\n... and ${results.length - maxShow} more files` : ''}`
